...
function submitAttendance() {
  ...
  const isAttendanceSubmitted = isAttendanceSubmittedForClass(selectedClass);
...
function showAttendanceResult(selectedClass) {
  ...
  const totalStudents = filteredAttendanceData.reduce((acc, record) => {
  ...
  const totalLeave = filteredAttendanceData.filter(
...
function showAttendanceResult(selectedClass) {
  ...
  // Update the result section
  const resultContent = `Date: ${date} | Time: ${time} | 
  ...
  const studentsListHTML = generateStudentsListHTML(filteredAttendanceData);
...
function updateAttendanceRecord(studentName, selectedClass, status) {
  ...
  const existingRecordIndex = savedAttendanceData.findIndex(
    (record) => record.name === studentName && record.class === selectedClass
  );
...
function saveClasses() {
  ...
  const classSelector = document.getElementById("classSelector");
  const savedClasses = Array.from(classSelector.options).map(
    (option) => option.value
  );
...
function saveStudentsList(selectedClass) {
  ...
  const studentsList = document.getElementById("studentsList");
  const savedStudents = JSON.parse(localStorage.getItem("students")) || {};
  const selectedClassStudents = Array.from(studentsList.children).map(
    (item) => {
      return {
...
function getSavedColor(selectedClass, rollNumber) {
  ...
  return savedColors[selectedClass]
    ? savedColors[selectedClass][rollNumber]
    : null;
}
